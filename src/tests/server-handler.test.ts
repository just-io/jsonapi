...
describe('ServerHandler', () => {
    ...
    test('should return error of invalid query', async () => {
        ...
        const { status, body } = await serverHandler.handle(
            context,
            'GET',
            'www.example.com/api/v1/users/10/relationship',
            '',
        );
    ...
    test('should return operation results', async () => {
        ...
        assert(body && typeof body === 'object' && 'atomic:results' in body && Array.isArray(body['atomic:results']));

        assert(
            body['atomic:results'][0] !== null &&
                typeof body['atomic:results'][0] === 'object' &&
                body['atomic:results'][0].data &&
                'id' in body['atomic:results'][0].data,
        );
        const noteId = body['atomic:results'][0].data.id;
        assert(
            body['atomic:results'][1] !== null &&
                typeof body['atomic:results'][1] === 'object' &&
                body['atomic:results'][1].data &&
                'id' in body['atomic:results'][1].data,
        );
        const tagId = body['atomic:results'][1].data.id;
